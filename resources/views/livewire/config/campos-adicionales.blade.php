...
$mockCampos = [
    // Empty array to show "No hay Registros para mostrar" as seen in mockup
];
...
state([
    ...
    'filtro_clientes' => 'Clientes',
]);

$filteredCampos = function () {
    $filtered = collect($this->campos);

    if ($this->filtro_estado === 'Activos') {
        $filtered = $filtered->where('estatus', 'Activo');
    } elseif ($this->filtro_estado === 'Inactivos') {
        $filtered = $filtered->where('estatus', 'Inactivo');
    }

    // Add filtering logic for Clientes dropdown if needed in the future

    if (!empty($this->search)) {
        $searchQuery = strtolower(trim($this->search));
        $filtered = $filtered->filter(function ($campo) use ($searchQuery) {
            return str_contains(strtolower($campo['campo']), $searchQuery) ||
                   str_contains(strtolower($campo['tipo_valor']), $searchQuery);
        });
    }
...
<div class="h-full bg-transparent p-6 font-sans">
    <div class="max-w-[1400px] mx-auto bg-white dark:bg-gray-800 rounded shadow-sm">
        ...
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700">
            <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">Campos adicionales</h1>
        </div>

        {{-- Toolbar --}}
        <div class="px-6 py-3 flex flex-col md:flex-row md:items-center justify-between border-b border-gray-100 dark:border-gray-700 gap-4">
            {{-- Left: Dropdowns --}}
            <div class="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                ...
                <div class="w-full md:w-48 relative">
                    <select wire:model.live="filtro_clientes" class="appearance-none w-full bg-transparent border-0 border-b border-gray-300 dark:border-gray-600 px-0 py-2 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-0 focus:border-gray-500 cursor-pointer">
            ...
            <div class="flex items-center gap-4 text-gray-500">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    <button class="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-gray-600 dark:text-gray-400 transition cursor-not-allowed" title="Exportar">
            ...
            <table class="min-w-full text-left bg-white dark:bg-gray-800">
                <thead class="bg-gray-50 dark:bg-gray-700 border-b border-gray-100 dark:border-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-[13px] font-bold text-gray-700 dark:text-gray-200 tracking-wide">Campo</th>
                        <th class="px-6 py-4 text-[13px] font-bold text-gray-700 dark:text-gray-200 tracking-wide">Requerido</th>
                        <th class="px-6 py-4 text-[13px] font-bold text-gray-700 dark:text-gray-200 tracking-wide">Tipo Valor</th>
                        <th class="px-6 py-4 text-[13px] font-bold text-gray-700 dark:text-gray-200 tracking-wide">Estatus</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($this->filteredCampos() as $campo)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                        ...
                        {{-- Data --}}
                        <td class="px-6 py-3 text-[13px] text-gray-800 dark:text-gray-300 whitespace-nowrap">{{ $campo['campo'] }}</td>
                        <td class="px-6 py-3 text-[13px] text-gray-800 dark:text-gray-300 whitespace-nowrap">{{ $campo['requerido'] ? 'Sí' : 'No' }}</td>
                        <td class="px-6 py-3 text-[13px] text-gray-800 dark:text-gray-300 whitespace-nowrap">{{ $campo['tipo_valor'] }}</td>
                        <td class="px-6 py-3 text-[13px] text-gray-800 dark:text-gray-300 whitespace-nowrap">{{ $campo['estatus'] }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-2 text-center text-gray-700 dark:text-gray-400 text-sm font-medium">
        ...
        <div class="px-6 py-3 flex items-center justify-end text-sm text-gray-600 dark:text-gray-400 border-t border-gray-100 dark:border-gray-700 gap-6">
            ...
            <div class="flex items-center gap-4">
                ...
                @if(count($this->filteredCampos()) > 0)
                    <span class="text-[13px] mx-1">1-{{ count($this->filteredCampos()) }} of {{ count($this->filteredCampos()) }}</span>
                @else
