...
$mockRoles = [
    ['id' => 1, 'rol' => 'Super Administrador', 'fecha' => '2024-11-25 15:42', 'estatus' => 'Habilitado'],
    ['id' => 2, 'rol' => 'Administrador', 'fecha' => '2025-05-09 10:16', 'estatus' => 'Habilitado'],
    ['id' => 3, 'rol' => 'Supervisor', 'fecha' => '2025-06-12 11:30', 'estatus' => 'Habilitado'],
    ['id' => 4, 'rol' => 'Vendedor', 'fecha' => '2026-01-10 09:15', 'estatus' => 'Deshabilitado'],
];
...
$filteredRoles = function () {
    ...
    if (!empty($this->search)) {
        ...
        $filtered = $filtered->filter(function ($rol) use ($searchQuery) {
            return str_contains(strtolower($rol['rol']), $searchQuery);
        });
    }
    ...
    <div class="max-w-[1400px] mx-auto bg-white rounded shadow-sm">
                    ...
                    <tr class="hover:bg-gray-50 transition duration-150 group">
                        ...
                        <td class="px-6 py-3 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                {{-- Si NO es super administrador, mostramos eliminar --}}
                                @if($rol['rol'] !== 'Super Administrador')
                                ...
                                {{-- Modulos icon (grid) solo para roles normales --}}
                                @if($rol['rol'] !== 'Super Administrador')
                                <div class="relative flex items-center justify-center">
                                    <button class="bg-gray-200 p-1 rounded-full text-gray-600 hover:bg-gray-300 hover:text-gray-800 transition peer" title="Módulos">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M4 4h4v4H4V4zm6 0h4v4h-4V4zm6 0h4v4h-4V4zM4 10h4v4H4v-4zm6 0h4v4h-4v-4zm6 0h4v4h-4v-4zM4 16h4v4H4v-4zm6 0h4v4h-4v-4zm6 0h4v4h-4v-4z" />
                                        </svg>
                                    </button>
                                    {{-- Tooltip Módulos --}}
                                    <div class="absolute top-full left-1/2 -translate-x-1/2 mt-2 px-2.5 py-1 bg-gray-600 text-white text-[10px] rounded opacity-0 peer-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10">
                                        Módulos
                                    </div>
                                </div>
                                @endif
        ...
        <div class="px-6 py-3 flex items-center justify-end text-sm text-gray-600 border-t border-gray-100 gap-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <span class="text-[13px] mx-1">1-{{ count($this->filteredRoles()) }} of {{ count($this->filteredRoles()) }}</span>
