...
$mockZonas = [
    ['id' => 1, 'zona' => 'Zona 1', 'clave' => '1Z', 'estatus' => 'Activo'],
];
...
$filteredZonas = function () {
    $filtered = collect($this->zonas);
    ...
    if (!empty($this->search)) {
        ...
        $filtered = $filtered->filter(function ($zona) use ($searchQuery) {
            return str_contains(strtolower($zona['zona']), $searchQuery) ||
                   str_contains(strtolower($zona['clave']), $searchQuery);
        });
    }
...
<div class="h-full bg-transparent p-6 font-sans">
    <div class="max-w-[1400px] mx-auto bg-white dark:bg-gray-800 rounded shadow-sm">
        ...
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700">
            <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">Zonas</h1>
        </div>
            ...
            <table class="min-w-full text-left bg-white dark:bg-gray-800">
                <thead class="bg-gray-50 dark:bg-gray-700 border-b border-gray-100 dark:border-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-[13px] font-bold text-gray-700 dark:text-gray-200 tracking-wide">Zona</th>
                        <th class="px-6 py-4 text-[13px] font-bold text-gray-700 dark:text-gray-200 tracking-wide">Clave</th>
                        <th class="px-6 py-4 text-[13px] font-bold text-gray-700 dark:text-gray-200 tracking-wide">Estatus</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($this->filteredZonas() as $zona)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                        {{-- Acciones --}}
                        <td class="px-6 py-3 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                {{-- Map Pin (Ubicación) icon --}}
                                <button class="text-green-500 hover:text-green-700 transition" title="Ubicación">
                        ...
                        {{-- Data --}}
                        <td class="px-6 py-3 text-[13px] text-gray-800 dark:text-gray-300 whitespace-nowrap">{{ $zona['zona'] }}</td>
                        <td class="px-6 py-3 text-[13px] text-gray-800 dark:text-gray-300 whitespace-nowrap">{{ $zona['clave'] }}</td>
                        <td class="px-6 py-3 text-[13px] text-gray-800 dark:text-gray-300 whitespace-nowrap">{{ $zona['estatus'] }}</td>
                    </tr>
                    @empty
                    <tr>
        ...
        <div class="px-6 py-3 flex items-center justify-end text-sm text-gray-600 dark:text-gray-400 border-t border-gray-100 dark:border-gray-700 gap-6">
            ...
            <div class="flex items-center gap-4">
                ...
                @if(count($this->filteredZonas()) > 0)
                    <span class="text-[13px] mx-1">1-{{ count($this->filteredZonas()) }} of {{ count($this->filteredZonas()) }}</span>
                @else
